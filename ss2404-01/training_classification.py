...
def training(
    ...
    model = WideResNet18(in_channels, class_num, k=2)
    ...
    plt.plot(range(1, max_epoch_num + 1), train_losses, label="Training Loss")
    plt.plot(range(1, max_epoch_num + 1), valid_losses, label="Validation Loss")
    plt.xlabel("Epochs")
    plt.ylabel("Loss")
    plt.title("Training and Validation Loss")
    plt.legend()
    plt.grid()
    plt.savefig(f"{output_path}/learning_curve_{time_stamp}.png")
    plt.show()
...
def main():
    ...
    for _ in range(1):
        tmp_value = np.random.rand()
        rand_value = 10 ** (-3.0 - (3.0 * tmp_value))

        print(f"Learning Rate: {rand_value}")
        results = training(
            args.training_data_path,
            args.validation_data_path,
            args.output_path,
            rand_value,  # args.learning_rate
            args.beta_1,
            args.batch_size,
            args.max_epoch_num,
            args.gpu_id,
